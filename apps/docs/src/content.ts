...
export const PAGES: Record<string, DocPage> = {
  ...
  "artifact-chat-container": {
    ...
    markdown:
      "# ArtifactChatContainer\n\nA container-aware chat and artifact composition. At larger widths the full chat docks on the left and the artifact fills the right. Below the breakpoint, the artifact keeps the whole canvas and the composer becomes a dark, translucent glass overlay floating above it — the page stays visible and legible underneath. A small grabber cap sits fused to the top edge of that overlay. Tapping or dragging the cap grows the conversation upward out of it: one continuous surface that gets taller and pushes the cap up, never a second card fading in over the composer.\n\n```tsx\nimport { ArtifactChatContainer } from '@touchkit/chatkit'\n\n<ArtifactChatContainer\n  breakpoint={760}\n  chatWidth={400}\n  working={isWorking}\n  workingLabel=\"Working on the artifact…\"\n  onAdd={() => setIsWorking(false)}\n>\n  <ArtifactChatContainer.Chat><Conversation /></ArtifactChatContainer.Chat>\n  <ArtifactChatContainer.Composer><Composer /></ArtifactChatContainer.Composer>\n  <ArtifactChatContainer.Content><Artifact /></ArtifactChatContainer.Content>\n</ArtifactChatContainer>\n```\n\n## Responsive slots\n\n| Slot | Split layout | Compact layout |\n| --- | --- | --- |\n| `Chat` | Full left column | Grows upward inside the floating overlay, bottom-pinned to the composer |\n| `Composer` | Bottom of the chat column | The floating overlay itself |\n| `Content` | Right column | Full canvas, visible through the overlay |\n\nThe breakpoint measures the container, not the viewport. The cap is a real drag target: dragging it tracks the pointer one-to-one so the conversation follows your finger out of the cap, and releasing past roughly a third of the travel settles it open. A plain tap toggles. The scrim and `Escape` both collapse it. `chatOpen` / `onChatOpenChange` provide controlled state; `useArtifactChatContainer()` exposes the measured width and presentation state to descendants.\n\nAs the surface grows it deepens — the glass darkens, the shadow spreads, and a hairline appears above the composer — so the light floating composer and the settled full conversation are the same object at two points in one animation.\n\n## Working and scroll behavior\n\nWhen `working` is true, the composer row becomes a translucent, tappable **Working…** bar inside the same overlay. Tapping it reveals the same composer so the user can add another request without leaving the artifact. Use `onAdd` to clear the working state, then set `working` again when the new request begins. The editor stays mounted in one presentation at a time, so it never appears twice.\n\n`hideOnScroll` defaults to true. The collapsed overlay subscribes to the same TouchKit chrome state as `TabBar`, so a `List` scrolling down moves the header, tab bar, and artifact chat together; scrolling up returns all three. Once the chat is expanded it stops following scroll chrome.\n\n## Live example\n\nSwitch between split and floating layouts, drag the cap to grow the chat out of the composer, and preview the working state.\n\n%%live:artifactchat%%",
  },
  "chat-shell": {
    id: "chat-shell",
    section: "Containers",
