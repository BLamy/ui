...
export const PAGES: Record<string, DocPage> = {
  ...
  "artifact-chat-container": {
    ...
    markdown:
      "# ArtifactChatContainer\n\nA container-aware chat and artifact composition. At larger widths the full chat docks on the left and the artifact fills the right. Below the breakpoint, the artifact keeps the canvas while the composer floats above its bottom edge. The grabber opens the full conversation over the artifact.\n\n```tsx\nimport { ArtifactChatContainer } from '@touchkit/chatkit'\n\n<ArtifactChatContainer\n  breakpoint={760}\n  chatWidth={400}\n  working={isWorking}\n  workingLabel=\"Working on the artifact…\"\n  onAdd={() => setIsWorking(false)}\n>\n  <ArtifactChatContainer.Chat><Conversation /></ArtifactChatContainer.Chat>\n  <ArtifactChatContainer.Composer><Composer /></ArtifactChatContainer.Composer>\n  <ArtifactChatContainer.Content><Artifact /></ArtifactChatContainer.Content>\n</ArtifactChatContainer>\n```\n\n## Responsive slots\n\n| Slot | Split layout | Compact layout |\n| --- | --- | --- |\n| `Chat` | Full left column | Full-height drawer above the composer |\n| `Composer` | Bottom of the chat column | Floating over the artifact |\n| `Content` | Right column | Full canvas behind the chat |\n\nThe breakpoint measures the container, not the viewport. The compact grabber can be tapped or dragged: up opens the full chat and down collapses it. `chatOpen` / `onChatOpenChange` provide controlled drawer state; `useArtifactChatContainer()` exposes the measured width and presentation state to descendants.\n\n## Working and scroll behavior\n\nWhen `working` is true, the compact dock becomes a translucent, tappable **Working…** bar. Tapping it reveals the same composer so the user can add another request without leaving the artifact. Use `onAdd` to clear the working state, then set `working` again when the new request begins. The editor stays mounted in one presentation at a time, so it never appears twice.\n\n`hideOnScroll` defaults to true. The floating dock subscribes to the same TouchKit chrome state as `TabBar`, so a `List` scrolling down moves the header, tab bar, and artifact chat together; scrolling up returns all three.\n\n## Live example\n\nSwitch between split and floating layouts, open the compact chat with its grabber, and preview the working state.\n\n%%live:artifactchat%%",
  },
  "chat-shell": {
    id: "chat-shell",
    section: "Containers",
