...
export const PAGES: Record<string, DocPage> = {
  ...
  "bui-streaming": {
    ...
    markdown:
      "# MarkdownStream\n\nA streamed answer rendered as real GitBook markdown by docstream (`@brett_lamy/docstream` via `@touchkit/workbench` MarkdownView) — superscript `[^1]` citations, a source-domain chip row, and `@mention` / `#tag` reference chips, all parsed from the stream itself.\n\n%%live:buiStreaming%%\n\n## Behavior\n\n- Streams ~14 words/second with slight jitter, like a real token stream.\n- `[^1]` markers render as superscript badges linking to their `[^1]: url` definitions; every defined source becomes a clickable domain chip under the answer.\n- `@name` and `#tag` render as reference chips — wire clicks with MarkdownView `onReferenceClick`.\n- A half-arrived token (`[^`, `@bre`) is held back while streaming, so chips never flash as plain text.\n- **Replay stream** restarts from zero.\n\nThis replaces the old hand-rolled `StreamingText` component — the chips are markdown now, not chrome.",
  },
  "bui-tool-chips": {
    id: "bui-tool-chips",
    section: "BUI · Agent state",
