...
export const PAGES: Record<string, DocPage> = {
  ...
  "docstream": {
    ...
    title: "Markdown",
    markdown:
      "# MarkdownView\n\nWorkbench chat replies and these docs use the package's local `MarkdownView`. It has no runtime CDN or parser dependency, so the first render and offline behavior are deterministic.\n\n```tsx\nimport { MarkdownView } from '@touchkit/workbench'\n\n<MarkdownView markdown={answer} />\n```\n\n## Supported syntax\n\n- Headings, paragraphs, emphasis, strong text, strike-through, links, and inline code\n- Ordered and unordered lists\n- Block quotes and horizontal rules\n- Tables\n- Fenced code blocks with lightweight JSX, TypeScript, JavaScript, HTML, XML, and SVG highlighting\n\nThe parser creates React elements and does not use `dangerouslySetInnerHTML`. Links open in a new tab with `rel=\"noreferrer\"`.\n\n## Streaming updates\n\nPass the latest accumulated markdown as chunks arrive. `streaming` is accepted for API compatibility, while the local parser renders the current string the same way in either state. Keep incomplete fences or tables in your stream buffer if your product needs syntax-aware partial-block behavior.\n\n```tsx\n<MarkdownView markdown={partialAnswer} streaming={isStreaming} />\n```\n\n## Styling\n\nImport `@touchkit/workbench/styles.css`. The rendered wrapper uses `data-slot=\"markdown-view\"` and class `wb-md`; pass `className` or `style` for layout-specific adjustments.\n\n## Live example\n\nThe demo feeds an answer in chunks so you can inspect partial and completed rendering:\n\n%%live:stream%%",
  },
  "pencilkit": {
    id: "pencilkit",
    section: "Drawing",
